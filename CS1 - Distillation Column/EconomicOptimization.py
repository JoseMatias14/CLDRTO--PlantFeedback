...
def CLDynamicEconomicOptimization(xvar,pvar,xdot):
    ...
    xD = xvar[2*par['NT']] # delayed state, without delay = par['NT'] - 1 
    # computing error
    eL = xD - pvar[0]
    ...
    for k in range(ecoPar['N']):
        ...
        g.append(Xk_end[2*par['NT']] + bias_est_var[0]) # g.append(Xk_end[par['NT'] - 1] + bias_est_var[0])
...
def CallCLDRTO(CLDRTOsolver,x0HatCLk,UkCL_1,thetaTraj,zTraj,biasHatk,biasCtrlk,xPurity,sysMeas,w_warm,lam_w_warm,lam_g_warm):
    ...
    ## TOP COMP. ##
    # Top composition
    xD = xTemp[2*par['NT']] #  # delayed state, without delay = par['NT'] - 1 
    # computing error
    eL = xD - UkCL_1[0]
    # Reflux flow # [kmol/min]
...
def SystemEcoDynOptBoundsCL(x0CLHat,bias0Hat,xPurity,sysMeas,zTraj):
    ...
    ##########
    # upper bounds
    x_fraction_LB = np.zeros((int((dx0.size - ctrlPar['crtlNum'] - 1)/2),1))  # -1 from the delayed measurement
    x_holdup_LB =   np.zeros((int((dx0.size - ctrlPar['crtlNum'] - 1)/2),1))
    x_delay_LB = np.zeros((1,1))
    x_E_LB = -100*np.ones((int(ctrlPar['crtlNum']),1))
    xLB = np.concatenate((x_fraction_LB,x_holdup_LB,x_delay_LB,x_E_LB))
    
    # upper bounds
    x_fraction_UB =   np.ones((int((dx0.size - ctrlPar['crtlNum'] - 1)/2),1))
    x_holdup_UB = 100*np.ones((int((dx0.size - ctrlPar['crtlNum'] - 1)/2),1))
    x_E_UB = 100*np.ones((int(ctrlPar['crtlNum']),1))
    x_delay_LB = np.ones((1,1))
    xUB = np.concatenate((x_fraction_UB,x_holdup_UB,x_delay_LB,x_E_UB))
    ...
    Hbias = par['HCL'].T.dot(par['b2m'].T)
    # Hbias = np.zeros((nx,nbias))
    # Hbias[2*par['NT'],0] = 1 # xD
    # Hbias[0,1] = 1 # xB
