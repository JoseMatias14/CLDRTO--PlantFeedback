...
#Period 2: low
period2 = 0.8*np.ones(((420)*12,))

dist1 = 0.5*np.concatenate((period1,period2))
...
#ax1.plot(timeArray,dist1Approx,'c:', linewidth=2)
ax1.set_ylabel('zF (A) [-]', fontsize=12)
ax1.set_title('Column feed (A fraction) disturbance', fontsize=12)
ax1.minorticks_on()
...
#ax2.plot(timeArray,dist2Approx,'c:', linewidth=2)
...
# saving figure 
if savePlot is True:
    fig.savefig("stepDisturbances.pdf", format="pdf", bbox_inches="tight")
    fig.savefig('stepDisturbances.tif', format='tif', dpi=600)
...
with open('disturbance_step.pkl', 'wb') as file:
