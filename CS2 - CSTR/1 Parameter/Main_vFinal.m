...
theta_m = [theta_p(1);
            ...
            theta_p(4);
            theta_p(5);
            -16;%theta_lb + (theta_ub - theta_lb)*rand(1,1);
            theta_p(7)];
...
for kk = 1:nEnd
    ...
    if kk > Ne
        ...
        if ma(1) == 1
            ...
            thetaDRTO = [theta_nom(1);theta_nom(2);theta_nom(3);theta_nom(4);theta_nom(5);theta_m(6);theta_nom(7)];
        ...
        elseif ma(2) == 1
            ...
            thetaDRTO = [theta_nom(1);theta_nom(2);theta_nom(3);theta_nom(4);theta_nom(5);thetaHatk;theta_nom(7)];
        ...
        if solverDRTO.stats.success ~=1
            ...
            % updating
            Uk_star = u_temp(:,1);
            Uk_p = [Uk_star(1);Uk_star(2);Uk_star(3)]; % all are implemented
    ...
    thetaPlantArray = [thetaPlantArray, theta_p(6)];
...
save(nameFile,'dt_sys','Ne','Np','tEnd','nEnd','XPlantArray','XMeasArray','UPlantArray','OFPlantArray','thetaPlantArray',...
...
function [X_0,U_0,theta_nom,sysBounds] = CSTRModelInitParBounds()
    ...
    % Parameters bounds
    sysBounds.theta_lb = -18;   % Psi [rearranged activation energy]

    sysBounds.theta_ub = -14;   % Psi [rearranged activation energy]
...
function [solver,MHEbounds,N,Qx,Qtheta,Qy] = MHE(F,theta_nom,dt_sys,dim_sys,sb)
    ...
    for k=0:N-1
        ...
        Yk = X(k+1, :)';
        Uk = U(k+1, :)';
        thetak = [theta_nom(1);theta_nom(2);theta_nom(3);theta_nom(4);theta_nom(5);Pk{1};theta_nom(7)];
...
function Pkk = ArrivalCost(Xhatk,thetaHatk,Pkk,Ukp,theta_p,dt_sys,dim_sys,Qx,Qtheta,Qy,F,S_xx,S_xp)
    ...
    F_xp(:,1:8) = [];%excluding derivative in relation to inputs
...
function PlotResults(nameFile)
    ...
    for ii = 1
        ...
        hold on
        for kk = Ne + 1:nEnd
            if SolMHEFlag(kk - Ne) == 1
                timeTemp = dt_sys*(kk - Ne):dt_sys:dt_sys*kk;
                plot(timeTemp,thetaHatTrajectory{kk},'Color',[0, 0, 0.01])
            end
        end
    ...
    figure(8)
    subplot(2,1,1)    
        plot(tsimgrid(Ne+2:end),SolMHEFlag,'kx','markersize',8)
    ...
    subplot(2,1,2)    
        plot(tsimgrid(Ne+2:end),SolDRTOFlag,'kx','markersize',8)
    ...
    figure(9)
    subplot(3,1,1)    
        plot(tsimgrid(Ne+2:end),execTimeArray(1,:),'kx-','markersize',5)
    ...
    subplot(3,1,2)    
        plot(tsimgrid(Ne+2:end),execTimeArray(2,:),'kx-','markersize',5)
    ...
    subplot(3,1,3)    
        plot(tsimgrid(Ne+2:end),100*execTimeArray(2,:)./(execTimeArray(1,:) + execTimeArray(2,:)),'kx-','markersize',5)
