...
%% save results
if ma(1) == 1
    nameFile = 'test3_BIAS_2p';
elseif ma(2) == 1
    nameFile = 'test3_MHE_2p';
end
...
for kk = 1:nEnd
    ...
    Fk = F('x0', Xk_p, 'p', [Uk_p;theta_p;dt_sys;zeros(dim_sys.nx - 1,1);0;0]);
    Xk_p = full(Fk.xf);
    OFk_p = full(Fk.qf);
    ...
    % Compute model state
    Fk = F('x0', Xk_m, 'p', [Uk_p;theta_m;dt_sys;zeros(dim_sys.nx - 1,1);0;0]);
    Xk_m = full(Fk.xf);
    ...
    thetaPlantArray = [thetaPlantArray, [theta_p(3);theta_p(6)]];
    ...
    if kk > Ne && rem(kk,dRTO_exec) == 0
        ...
        elseif ma(2) == 1
            ...
            thetaDRTO = [theta_nom(1);theta_nom(2);thetaHatk(1);theta_nom(4);theta_nom(5);thetaHatk(2);theta_nom(7)];
...
function [F,S_xx,S_xp] = CSTRDynamicModel()
    ...
    % bool for turning on and off input regulation term (if 1 consider DU
    % ter). Used mainly for computing OF at plant level for plotting
    boolU = SX.sym('boolU'); 

    % Economic term
    % conversion: moles of A reacted vs. moles of A fed
    L = (1 - (C_A + bias_m{1})/CAf) - boolU*DU_Caf*(1.3*1e-2)*DU_Caf; 
    ...
    % CREATING INTEGRATOR %
    %%%%%%%%%%%%%%%%%%%%%%%
    ode = struct('x',x,'p',vertcat(u,theta,bias_m,DU_Caf,boolU),'ode',h*xdot,'quad',h*L); 
...
function [X_0,U_0,theta_nom,sysBounds] = CSTRModelInitParBounds()
    ...
    sysBounds.theta_lb = [-7;
                          -18];   % Psi [rearranged activation energy]

    sysBounds.theta_ub = [-3;
                          -14];   % Psi [rearranged activation energy]
...
function [solver,MHEbounds,N,Qx,Qtheta,Qy] = MHE(F,theta_nom,dt_sys,dim_sys,sb)
    ...
    for k=0:N-1
        ...
        Yk = X(k+1, :)';
        Uk = U(k+1, :)';
        thetak = [theta_nom(1);theta_nom(2);Pk{1};theta_nom(4);theta_nom(5);Pk{2};theta_nom(7)];
    
        % Integrate till the end of the interval
        Fk = F('x0',Xk,'p', [Uk;thetak;dT_MHE;zeros(2,1);0;1]); % last two inputs are dummy here (no bias and DU is used only in the economic layer)
        Xk_end = Fk.xf;
...
function Pkk = ArrivalCost(Xhatk,thetaHatk,Pkk,Ukp,theta_p,dt_sys,dim_sys,Qx,Qtheta,Qy,F,S_xx,S_xp)
    ...
    Fk = F('x0', Xhatk, 'p', [Ukp;thetaHatLong;dt_sys;zeros(dim_sys.nx - 1,1);0;1]);
    Xhatkk = full(Fk.xf);

    % last two inputs are dummy (only for computing economic OF)
    F_xx = full(S_xx(Xhatkk,[Ukp;thetaHatLong;dt_sys;zeros(dim_sys.nx - 1,1);0;1]));
    F_xp = full(S_xp(Xhatkk,[Ukp;thetaHatLong;dt_sys;zeros(dim_sys.nx - 1,1);0;1]));
    F_xp(:,10:15) = [];%excluding derivative in relation to par.T and bias
...
function [solver,DRTObounds,N] = DRTO(F,theta_nom,dt_sys,dim_sys,sb)
    ...
    for k=0:N-1
        ...
        % Integrate till the end of the interval
        Fk = F('x0',Xk,'p', [Uk;par;dT_DRTO;bias;DU;1]);
        Xk_end = Fk.xf;
        J = J + Fk.qf;
