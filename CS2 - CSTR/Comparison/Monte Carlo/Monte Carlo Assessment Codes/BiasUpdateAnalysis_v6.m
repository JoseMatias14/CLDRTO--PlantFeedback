...
%% save results
nameFile = 'MC_parameters_CLv3_med.mat';
...
parIndex = [3, 6]; % position correspondence with theta0
nPar = 2; % dH, \Psi
...
for ii = 1:nPar
    for jj = 1:3 + nSim
        ...
        for kk = 1:nEnd
            ...
            OFPlantArray = [OFPlantArray, 1 - Xk_p(1)/Uk_p(1)];
            ...
            if kk > 10 && rem(kk,dRTO_exec) == 0
                ...
                thetaDRTO = [theta_nom(1);theta_nom(2);theta_m(3);theta_nom(4);theta_nom(5);theta_m(6);theta_nom(7)];
